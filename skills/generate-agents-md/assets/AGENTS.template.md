...
- The Dispatcher owns requirement clarification, recommended design, task decomposition, dependency/authorization coordination, returned-problem diagnosis, repair routing, independent acceptance and final requirement reconciliation. It must not merely forward messages. A delegated Agent executes only its assigned boundary, may return evidence-backed technical objections, and cannot change requirements, scope or authority.
- Keep small work small. A small task uses one writer and targeted verification when impact is known and no behavior, contract or flow changes. Standard work uses at most one writer plus one read-only BLACK_BOX Agent. Use the existing complete/high-risk path only for verified high risk, large concurrent modules or independent/compliance requirements; investigate unknown impact first.
- First assignment required fields: objective; requirement IDs or explicit success conditions; recommended approach; scope and prohibitions; inputs/dependencies; acceptance; actual writer/permission binding; candidate and baseline; return format; next action. Add interaction/version identifiers only when the active host or real multi-Agent coordination uses them. Omit inapplicable conditional fields instead of creating empty artifacts, tables or fictitious leases. References must name an accessible source and stable version/hash, never “previous chat”.
- Return a problem with facts, expected versus actual behavior, reproduction/evidence, known cause or bounded hypothesis, impact and blocking status. The Dispatcher classifies it as requirement, design, implementation, environment or permission; then issues one consolidated delta containing the minimum repair scope, regression condition, invalidated evidence and continue/pause/cancel action. Unknown causes get bounded investigation, not an assumed diagnosis. Users decide genuine requirement tradeoffs and necessary permissions, not technical decomposition.
- After assignment, communicate only material deltas, blockers and useful stage results. Do not require acknowledgements, heartbeat messages or unchanged polling. Use bounded waits and idempotent retry of the same event; timeout means unknown, not failed, and never authorizes a second writer. Late, cancelled, stale-version or wrong-candidate results cannot advance status. Stop or authority-revocation instructions always take effect.
- Reuse the existing delivery contract and registered scope/check/plan/gate/record commands. A protocol/document check is not host delivery, execution or acceptance. Evidence must exist and match the same requirement, run, candidate and independent role; a non-empty reference or Agent self-report cannot pass a gate. Scope/requirement/candidate change invalidates affected prior evidence. Merge source only after applicable same-candidate checks, independent acceptance and baseline-drift checks pass; release remains separately authorized.
- Sender/model text in a message is not runtime identity proof. Control must come through the designated Dispatcher and supported host. Default coordination does not add host signatures, daemons, network services, automatic approval or strict mode.
- Cost comparisons use actual metered input, cached-input and output tokens when available; missing attribution is `unavailable`. Message or character counts are not token usage. Any comparison of total tokens, elapsed time, supplemental dispatches, rework or missed requirements must be labelled unmeasured until a controlled run is executed.
