...
def main(argv: list[str] | None = None) -> int:
    try:
        arguments = build_parser(
            "Validate and activate a same-user local-controlled module write lease.",
            "activate",
        ).parse_args(argv)
        ...
        result = activate_signed_module_lease(payload, arguments.registry_path, now)
    except ValueError:
        ...
        return 1
    emit({"complete": result["complete"], "lease_id": result["lease_id"],
          "status": result["status"], "valid": True})
    return 0
