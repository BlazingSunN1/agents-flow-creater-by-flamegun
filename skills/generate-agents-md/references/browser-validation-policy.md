...
- 使用 `assets/frontend-evidence.template.json` 记录实时入口、预览根、入口工件与哈希、实时响应哈希、浏览器运行时 UTF-8 DOM 快照、CSS id 点击/断言目标、run/time/viewport、转录、PNG、argv 哈希和原生 E2E 报告；运行时 DOM 可与服务端入口字节不同，但必须绑定自身哈希和同一浏览器 run。
...
- completion 绑定当前原生独立 BLACK_BOX Agent；应用内浏览器转录由该 run 的真实点击导出，禁止实现者手写转录冒充执行。桌面 PC 默认适用；仅当批准范围包含移动 Web、触控或响应式浏览器行为时增加独立浏览器移动证据。原生移动执行登记的原生测试；跨端变更同时运行浏览器与原生两套门禁。
