...
- 所有任务先闭合最小可靠链：目标/范围/非目标/可量化验收 → 从真实入口跑通最小实现至可观测业务结果 → 受影响测试和现有相关静态检查 → 对照验收点记录当前证据。把首次通过结果的代码版本、Build ID、验收命令、结果和证据 SHA-256 冻结为可用基线；之后才启动非必要门禁扩展、重构、代码质量、性能或体验打磨。首个成果前仅前置运行正确或防不可逆损害所需检查；治理完整或门禁通过不能替代业务成果。打磨发生回归时先恢复最小闭环并重跑冻结验收，不得改需求或弱化验收换取通过。不适用环节只记可验证 `N/A` 理由，不生成空产物。
...
- 首个可用成果通过受影响验证并冻结后，再运行由变更面、批准范围或已核实失败模式映射的格式、类型、Lint、复杂度、安全和架构规则；无映射的通用门禁不得阻塞首个成果。
...
- 指纹未变化时避免重复执行相同命令；不得跳过当前交付阶段与风险映射明确要求的正确性、追踪、审查、泳道和验收门禁，也不得把未映射的通用安全门禁升级为首个成果的阻断项。
...
真实命令必须写入 `assets/project-commands.template.json` 对应的项目清单，包含 argv、声明来源、选择器、工作目录和适用性；首个成果可先运行从仓库事实核实的真实入口，但在生成 receipt、冻结基线或完成前必须运行 `scripts/validate_project_commands.py`，不得以恒定成功、Shell 包装、吞错或臆造命令代替。
...
- [ ] 已从真实入口跑通最小业务流程，冻结代码版本、Build ID、验收命令、可观测结果和证据 SHA-256；非必要门禁与优化在冻结后启动，且打磨候选已重放冻结验收、无回归。
