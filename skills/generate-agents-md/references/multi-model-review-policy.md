...
- 标准任务只调度一个实现/维护 Agent 和一个只读 `BLACK_BOX` Agent：前者显式指定 `model=gpt-6-astra`、`reasoning_effort=medium`，后者用同一模型与 `reasoning_effort=high` 合并变更审查、验收用例复核和真实黑盒。高风险循环新增的方案与 coordinator/adjudicator 角色同样为 `high` 且只读。不支持精确模型或推理强度时失败关闭，不替换或降级。
