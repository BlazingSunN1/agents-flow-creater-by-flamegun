# 原生 GPT-6 Astra 并列评审策略

The default implementation and repair writer is one native `gpt-5.6-sol/medium` Agent holding the unique active lease. Native `gpt-6-astra/high` Dispatcher, solution, review, adjudication, and acceptance roles are read-only. [Local Qwen code author](local-qwen-code-author.md) is an explicit user-selected alternative writer policy and is never an automatic fallback.
...
- 标准任务只调度一个实现/维护 Agent 和一个只读 `BLACK_BOX` Agent：前者显式指定 `model=gpt-5.6-sol`、`reasoning_effort=medium`，后者指定 `model=gpt-6-astra`、`reasoning_effort=high`，合并变更审查、验收用例复核和真实黑盒。高风险循环新增的方案与 coordinator/adjudicator 角色同样为 GPT-6/high 且只读。不支持精确模型或推理强度时失败关闭，不替换或降级。
