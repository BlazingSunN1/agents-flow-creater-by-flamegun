...
### 3.1 候选代码先验收、后写入正式工作区

- “落盘”指将实现或修复代码（含测试、脚本及配置即代码）写入正式工作区；候选代码、测试产物和证据可写入本次任务独占的隔离候选目录。不得先覆盖正式文件再测试。
- 生成候选前，根据方案确定验收条件、适用测试、允许修改路径和正式文件基线哈希；保留现有未提交改动，不得为使候选通过而擅自削弱验收条件。
- 代码生成继续遵守现有作者、模型和租约规则；先在隔离候选区应用并测试。测试不得修改正式工作区、共享业务数据或生产环境；无法隔离或缺少测试执行授权时阻断正式代码写入。
- 只有同一候选的全部适用测试（含适用的独立黑盒）通过，才允许当前唯一租约写者应用到正式工作区。证据须绑定方案、候选内容哈希、测试命令、环境、退出状态和结果；失败、未执行、未完成或证据不匹配均不满足条件。
- 应用前重新检查授权路径、正式文件基线哈希及已验收候选哈希；正式文件被其他任务修改、候选发生变化或证据失效时停止应用，不得强制覆盖或复用旧结果。只应用已验收的同一候选内容。
- 不满足条件则不应用，并丢弃本次候选改动，保留必要失败证据和未完成状态。清理仅限本次任务独占的候选产物，不删除或回滚正式文件、用户改动及其他任务数据；修订须作为新候选重新验收。
